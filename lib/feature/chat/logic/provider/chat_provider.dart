...
class ChatProvider with ChangeNotifier {
  ...
  Stream<QuerySnapshot> searchUsers(String query) {
    return _firestore
        ...
        .where('email', isLessThanOrEqualTo: query + "/uf8ff")
        .snapshots();
  }
  Future<void> sendMessage(String chatId, String message, String receivedId)async{
    final currentUser = _auth.currentUser;
    if(currentUser!=null){
      await _firestore.collection('chats').doc(chatId).collection('message').add({
        'senderId': currentUser.uid,
        'receverId': receivedId,
        'messageBody': message,
  ...
  Future<String?> getChatRoom (String receivedId)async{
    ...
    if(currentUser !=null){
      ...
      final chats = chatQuery.docs.where((chat) => chat['users'].contains(receivedId)).toList();
      if(chats.isNotEmpty){
